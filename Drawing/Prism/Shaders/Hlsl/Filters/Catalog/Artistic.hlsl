...
float2 CatalogStructureTensorTangent(float3 tensor)
{
    ...
    if (energy <= 0.000001 || discriminant <= 0.0000001)
    {
        return float2(1.0, 0.0);
    }

    float lambda2 = 0.5 * (energy - discriminant);
    float2 first = float2(tensor.y, lambda2 - tensor.x);
    float2 second = float2(lambda2 - tensor.z, tensor.y);
    float2 tangent = dot(first, first) >= dot(second, second)
        ? first
        : second;
    ...
    uint jitterSeed)
{
    ...
    float2 tangent = CatalogStructureTensorTangent(tensor);
    if (roughness > 0.0)
    {
        ...
        float jitter = DryBrushHash(
            ...
            jitterSeed) - 0.5;
        float jitterAngle =
            jitter * roughness * 3.14159265358979323846;
        float jitterCosine = cos(jitterAngle);
        float jitterSine = sin(jitterAngle);
        tangent = float2(
            (tangent.x * jitterCosine) -
                (tangent.y * jitterSine),
            (tangent.x * jitterSine) +
                (tangent.y * jitterCosine));
    }
    if (diagonalBias > 0.0)
    {
        float2 diagonalTangent;
        if (balanceDiagonalsByLuminance > 0.5)
        {
            float threshold = 1.0 - saturate(diagonalBalance);
            diagonalTangent = CatalogLuminance(source) >= threshold
                ? float2(diagonal, diagonal)
                : float2(diagonal, -diagonal);
        }
        else
        {
            diagonalTangent = tangent.x * tangent.y >= 0.0
                ? float2(diagonal, diagonal)
                : float2(diagonal, -diagonal);
        }
    ...
    float cosine = tangent.x;
    float sine = tangent.y;
    float majorRadius =
