...
{
    return tex2D(SpriteTextureSampler, ConteCrayonClampUv(uv));
}

float4 ConteCrayonOriginal(float2 uv, int profile)
{
    return WorkingAssociatedToLinearSrgb(
        tex2D(ConteCrayonOriginalSampler, ConteCrayonClampUv(uv)),
        profile);
}
...
{
    return frac(
        sin(dot(coordinate, float2(12.9898, 78.233)) + (seed * 17.17)) *
        43758.5453);
}
