...
internal static class MonoGameStrokeMeshBuilder
{
        ...
        Func<DrawPoint, XnaColor> colorSelector)
    {
        ...
        IReadOnlyList<DrawStrokeContour> contours = CreateContours(
            command,
            coordinateScale);
        DrawStrokeMesh stroke = DrawStrokeTessellator.Tessellate(
            contours,
            thickness,
            style);
        if (stroke.IsEmpty)
        ...
        VertexPositionColor[] vertices = new VertexPositionColor[stroke.Vertices.Length];
        ...
        return new MonoGameStrokeMesh(
            new MonoGamePathMesh(vertices, stroke.Indices),
            left,
            top);
    }
        ...
        float coordinateScale)
    {
        ...
        IReadOnlyList<DrawStrokeContour> flattened =
            DrawPathFlattener.FlattenStroke(path, tolerance);
        ...
        for (int contourIndex = 0; contourIndex < flattened.Count; contourIndex++)
        {
            ...
            for (int pointIndex = 0; pointIndex < points.Length; pointIndex++)
            {
                ...
                points[pointIndex] = new DrawPoint(
                    command.Rect.X +
                        ((point.X - command.SourceRect.X) * scaleX),
                    command.Rect.Y +
                        ((point.Y - command.SourceRect.Y) * scaleY));
            }
            mapped[contourIndex] = new DrawStrokeContour(
                points,
                contour.IsClosed);
        }
