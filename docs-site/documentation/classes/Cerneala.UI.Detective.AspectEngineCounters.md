...
Source: `UI/Detective/AspectEngineCounters.cs`
...
AspectEngineCounters counters = root.Detective.AspectCounters;
int considered = counters.RulesConsidered;
int matched = counters.RulesMatched;
int conditions = counters.ConditionEvaluations;
int declarations = counters.DeclarationsResolved;
```
...
AspectEngineCounters counters = root.Detective.CaptureAspect(button).Counters;
int tokenLookups = counters.TokenLookups;
```

## Remarks

`AspectEngineCounters` is a mutable counter holder produced by the root-owned `AspectEngine` and exposed through `Detective`. The public constructor creates a zero-valued counter set. The properties have public getters and internal setters, so callers can inspect values but only code inside the assembly updates them.

`AspectEngine.Resolve` increments `RulesConsidered` for every rule in the catalog, `ConditionEvaluations` once for every condition node actually evaluated after structural filtering, `RulesMatched` for rules whose target and conditions match, `DeclarationsResolved` for declarations on matched rules, and `TokenLookups` by the number of token dependencies on each resolved declaration. These counters are cumulative for the owning engine instance; applying or resolving another element keeps increasing the same object returned by `Detective.AspectCounters`.
