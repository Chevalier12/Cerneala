...
Without a template child, the fallback renderer fills the track bounds with `Background` and draws a border using the maximum side of `BorderThickness`. The constructor initializes `Background` to `Color(225, 225, 225)`, `BorderColor` to `Color(120, 120, 120)`, `BorderThickness` to `new Thickness(1)`, `SmallChange` to `0.1f`, and `LargeChange` to `1`.
...
| `BorderColor` | `Color` | `Control` | Gets or sets the fallback border color rendered by the track. The constructor sets it to `Color(120, 120, 120)`. |
