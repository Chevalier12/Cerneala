...
Provides the typed code API for the Prism `Clouds` filter.

```csharp
public sealed class CloudsFilter : PrismFilter
```
...
| --- | --- |
| `CloudsFilter()` | Creates the operation with Prism catalog defaults. |
...
Parameter assignments are validated against the `Clouds` catalog entry. Add the operation to a `PrismPipeline` or pass it directly to `Prism.Apply`.
