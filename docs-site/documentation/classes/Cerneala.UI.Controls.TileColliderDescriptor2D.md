...
Offsets and shape coordinates use tile-local destination units. The adapter applies the shape offset, `LocalTransform`, diagonal/horizontal/vertical cell flips, cell placement, layer offset, and inherited scene transforms in that order. Camera culling does not by itself determine collision residency: [explicit collision regions](Cerneala.UI.Controls.SceneCollisionRegion2D.md) and active simulated entities retain the required source payloads and adapters off camera. Adapters with no remaining residency interest can be released. Prepare required regions before synchronous collision queries; missing data is not treated as empty terrain.
