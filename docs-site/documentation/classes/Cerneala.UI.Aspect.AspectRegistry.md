...
| `BuildCatalog()` | `AspectCatalog` | Returns the cached catalog snapshot for the current registry version, rebuilding it after package changes. Throws `InvalidOperationException` when called from outside the registry's owning thread. |
