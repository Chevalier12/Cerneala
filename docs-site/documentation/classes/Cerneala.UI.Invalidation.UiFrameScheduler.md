...
When work exists, `ProcessFrame` runs phases in this order: inherited properties, command state, aspect, inherited properties again, motion before layout, measure, arrange, motion after layout, motion before render, render cache, hit test, and motion end frame. Each phase processes a snapshot of its queue. The inherited-property phase takes further snapshots until its queue is empty, allowing callback-generated inherited work to complete in the same frame. The default `UIRoot` processor consumes descendant inherited work as it begins recursive propagation, and the scheduler skips snapshot entries that are no longer queued. A later invalidation can requeue an element for another pass. Other phases do not repeatedly drain newly queued same-phase work; downstream phase work can still run in the current frame if that downstream snapshot has not been taken yet.
