...
`AspectDiagnostics.Snapshot` is the nested diagnostics payload produced by the root-owned Aspect engine. `AspectEngine.Apply` builds a snapshot from the resolved aspect, matched and rejected resolution steps, token traces, and the current `AspectEngineCounters` snapshot. `Detective.CaptureAspect` returns the stored snapshot for an element, or a new empty snapshot when the element has no stored aspect diagnostics.
