...
Coordinates a retained `UIRoot` with input, frame scheduling, motion invalidation, cursor publishing, and retained rendering.
...
`UiHost` is the core retained UI host. It owns the current `UIRoot` reference, applies viewport data to that root, dispatches `InputFrame` values through an `ElementInputBridge`, runs scheduled frame work, commits retained render data, and stores the last produced `UiFrame`.

The constructor can receive an initial root, viewport, input source, backend, clock, input bridge, and platform services through `UiHostOptions`. When a root is attached, the host applies platform services and viewport values to it. `SetRoot(UIRoot)` replaces the retained root, reapplies the same platform services and viewport, and forces the next update to invalidate the initial frame.

`Update(UiViewport?, TimeSpan?)` reads input from `InputSource`, or from `Backend.InputSource` when `InputSource` is not set. `Update(InputFrame, UiViewport?, TimeSpan?)` uses an explicit input frame. Both update paths require a retained root. During update, viewport changes invalidate measure, arrange, render, hit testing, and the subtree; the first frame is also invalidated so the root can produce retained render data.

`Draw()` uses `Backend.DrawingBackend`. `Draw(IDrawingBackend)` submits the root's retained renderer to an explicit drawing backend. Drawing requires a retained root and a drawing backend, but it does not create a new `UiFrame`; frames are produced by `Update`.
...
| `Backend` | Gets or sets the backend used as a fallback input source and by `Draw()`. |
...
| Name | Description |
| --- | --- |
| `SetRoot(UIRoot)` | Replaces the retained root, applies platform services and the current viewport, and marks the next update as the initial frame for that root. |
...
| `Draw()` | Draws through `Backend.DrawingBackend`. |
| `Draw(IDrawingBackend)` | Submits retained drawing commands to the specified drawing backend. |
...
| `Update(InputFrame, UiViewport?, TimeSpan?)` | `InvalidOperationException` | No retained root is attached. |
...
| `Draw(IDrawingBackend)` | `InvalidOperationException` | No retained root is attached. |
