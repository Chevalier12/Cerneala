...
`Update(UiViewport?, TimeSpan?)` reads input from `InputSource`, or from `Backend.InputSource` when `InputSource` is not set. `Update(InputFrame, UiViewport?, TimeSpan?)` uses an explicit input frame. Both update paths require a retained root and the root's owning UI thread. Each update first applies its supplied viewport and initial or time-sensitive invalidations, then drains one stable Relay snapshot before scheduler and input work. Relay callbacks therefore observe the current update's viewport, Relay invalidations can be processed in that update, and work posted during dispatch waits for the next update.

`Draw()` uses `Backend.DrawingBackend`. It analyzes the committed command list once, uses that internal result to decide whether backdrop is required, and acquires at most one lease from `Backend.BackdropFrameSource`. Every qualifying Prism scope shares that lease through one `DrawingFrameContext`. The host skips acquisition when analysis reports no visible, non-empty backdrop, submits a null lease when no provider is configured, and disposes an acquired lease in a `finally` path after backend submission.
