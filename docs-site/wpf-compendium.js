const comparisonRows = [
  ['Runtime', '`Application` + STA Dispatcher', '`GeneratedWindowApplication` / host-owned startup + root-owned `UiRelay`', 'different', 'Startup is host-specific. UiRelay provides bounded UI-thread marshaling, not WPF Application or Dispatcher semantics.'],
  ['Runtime', '`DispatcherObject` thread affinity', 'Attached UI, Motion and Aspects share root UI-thread affinity', 'different', 'Direct attached-UI mutation is fail-fast off-thread; explicit work uses `UiRelay.Post` or `InvokeAsync`.'],
  ['Runtime', 'Priority Dispatcher queues', 'FIFO `UiRelay` snapshot + `UiFrameScheduler` phase queues', 'different', 'Relay drains a bounded MPSC snapshot before retained phases; there is no DispatcherPriority pump.'],
  ['Runtime', '`Dispatcher.InvokeAsync` / `BeginInvoke`', '`UiRelay.InvokeAsync` / `Post`', 'close', 'The async-first shape transfers, but work is drained by host Update with explicit count/time budgets and FIFO snapshot semantics.'],
  ['Runtime', 'Dispatcher-owned `SynchronizationContext`', 'Scoped Relay synchronization context during Update and relay drain', 'different', 'Async continuations can return through the current root while the scope is active; Cerneala does not install a process-wide desktop dispatcher context.'],
  ...
  ['Data', 'Inherited `DataContext`', 'Retained `DataContext` + generated `$DataContext` bindings', 'close', 'A root `DataType` gives Roslyn the CLR type; nested paths reconnect, interpolate, and support explicit OneWay or TwoWay modes.'],
