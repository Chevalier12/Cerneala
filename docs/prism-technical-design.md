...
`PrismRendererOptions` expune limite configurabile pentru:

- bugetul hard total al tuturor suprafețelor Prism;
- bugetul soft și numărul maxim de intrări pentru cache-ul retained;
- dimensiunea maximă a unei suprafețe intermediare;
- numărul maxim de passes per compoziție.

Valorile livrate nu sunt inventate în TDD. Ele se stabilesc după benchmark pe
hardware-ul de referință și devin teste explicite. Testele unitare folosesc limite
mici injectate. Depășirea unei limite produce eviction al suprafețelor libere și,
dacă alocarea rămâne imposibilă, bypass sigur prin `PrismFallbackPolicy`. Nu există
un sistem adaptiv de calitate în prima implementare.
...
| Alocare suprafață eșuată | eviction al suprafețelor libere, apoi bypass |
...
`PrismDiagnostics` expune per frame:
...
## Schimbări de API public anticipate

- `IDrawingBackend.Render` primește `DrawingFrameContext`.
- `IUiBackend` expune `IBackdropFrameSource?`.
- `MonoGameUiHostOptions` acceptă source-ul backdrop și opțiunile Prism.
- `DrawCommandKind` primește scope-urile Prism.
- se adaugă tipurile publice strict necesare pentru composition definitions și
  profiles;
- Motion primește chei tipizate pentru proprietăți Prism.

Nu se adaugă API public pentru extensii third-party în această implementare.

La implementarea fiecărei schimbări publice trebuie actualizată documentația din
`docs-site/documentation/classes/` și manifestul aferent.
