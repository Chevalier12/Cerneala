...
public sealed class TiledContractCoverageTests : IDisposable
{
    ...
    [Fact]
    public void EmptySourcePreservesCatalogAndBoundsWithoutInventingARenderMap()
    {
        JsonObject source = Map();
        source["layers"] = new JsonArray();
        Scene2DImportResult result = Import(source);
    ...
    [Fact]
    public void EmptySourceStillRejectsAnOverlappingTileCatalog()
    {
        JsonObject source = Map();
        source["layers"] = new JsonArray();
        source["tilesets"]!.AsArray().Add(source["tilesets"]![0]!.DeepClone());
        Failure(Import(source), "SCN2D015");
    }
    ...
    public void ObjectsPreserveExactGeometryTransformsRolesAndBitsets()
    {
        ...
        TileColliderDescriptor2D oval = Assert.IsType<TileColliderDescriptor2D>(ellipse.Collider);
        ...
        Assert.Equal("0,0 16,0", fence.Points);
        TileColliderDescriptor2D segment = Assert.IsType<TileColliderDescriptor2D>(fence.Collider);
        Assert.Equal(TileColliderShape2D.Segment, segment.Shape);
        Assert.Equal([new Vector2(0, 0), new Vector2(16, 0)], segment.Vertices);
    ...
    public void TileCollisionObjectsComposeTheirLocalRotationAndGroupOffsetExactlyOnce()
    {
        ...
        Assert.True(result.Success, string.Join("; ", result.Diagnostics));
        TileColliderDescriptor2D descriptor = Assert.IsType<TileColliderDescriptor2D>(result.Document!.Levels[0].TileSets[0].Tiles[1].Collider);
    ...
    [Fact]
    [Trait("SceneImportStage", "2")]
    public void TileDefinitionRejectsASecondCollisionObject()
    {
        JsonObject map = Map("tiled-objects.tmj");
        JsonObject first = map["layers"]![2]!["objects"]![1]!.DeepClone().AsObject();
        JsonObject second = first.DeepClone().AsObject();
        second["id"] = 999;
        map["tilesets"]![0]!["tiles"]![0]!["objectgroup"] = new JsonObject
        {
            ["objects"] = new JsonArray(first, second)
        };

        Failure(Import(map), "SCN2D008");
    }
    ...
    public void TopDownOrderingIsStableAndGroupVisibilityDoesNotDestroySourceMetadata()
    {
        ...
        TileMap2DModel layer = level.TileMaps[2];
    ...
    public void ExternalTilesetAndFilePropertiesResolveRelativeToTheirOwnFileInsideRoot()
    {
        ...
        Scene2DImportResult result = Import(map);
        Assert.True(result.Success, string.Join("; ", result.Diagnostics));
        Assert.Equal("atlas.svg", result.Document!.Levels[0].TileSets[0].Properties["Info"]);
    ...
    [Fact]
    [Trait("SceneImportStage", "2")]
    public void SecondTileColliderStopsBeforeMaterializingEveryObject()
    {
        JsonObject map = Map("tiled-objects.tmj");
        ...
        for (int index = 1; index <= 4096; index++)
        {
            ...
            {
                ["id"] = index, ["width"] = 16, ["height"] = 16,
                ["x"] = index, ["y"] = 0, ["rotation"] = 0, ["visible"] = true
            });
        }
        ...
        Failure(result, "SCN2D008");
        Assert.True(allocated < 100_000_000, $"A rejected second tile collider allocated {allocated:N0} bytes.");
    }
