...
public sealed class PrismOperationalDiagnosticsTests
{
    ...
    public void SurfaceBudgetFailureReportsRequestedCurrentAndLimitBytes()
    {
        ...
        nint window = api.CreateWindow("prism-operational-budget", 96, 64, SdlWindowOptions.Hidden);
        ...
        using SdlGpuWindowGraphicsSession session = Assert.IsType<SdlGpuWindowGraphicsSession>(
            factory.Create(new SdlWindowSurface(window, api.GetWindowId(window)), 96, 64, 1));
        DiagnosticScene scene = CreateScene(ownerToken: 77);
        session.BeginFrame(Color.Transparent);
        api.FailTextureCreationAt = api.TextureCreationCount + 1;
        try
        {
            DrawingFrameContext context = new(scene.Analysis);
            session.DrawingBackend.Render(scene.Commands, in context);
        }
        finally
        {
        ...
        Assert.Contains($"hardByteLimit={new PrismRendererOptions().SurfaceHardByteLimit}", failure.Detail, StringComparison.Ordinal);
    }
