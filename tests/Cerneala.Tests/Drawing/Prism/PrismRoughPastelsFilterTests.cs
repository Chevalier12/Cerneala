...
public sealed class PrismRoughPastelsFilterTests
{
    [Fact]
    public void PlannerBuildsBoundedDeviceScaledAkfAndCanonicalSymbols()
    {
        ...
        Assert.Equal(18, plan.Passes[0].RadiusX);
        Assert.Equal(5.625f, plan.Passes[1].RadiusX);
        Assert.Equal(2, plan.GetOption("Texture").X);
        Assert.Equal(5, plan.GetOption("LightDirection").X);
        Assert.Equal(36, capped.Passes[0].RadiusX);
        Assert.Equal(11.25f, capped.Passes[1].RadiusX);
    }
    ...
    public void CpuReferenceIsDeterministicEdgeAwareAndHonorsEveryControl()
    {
        ...
        PrismPremultipliedColor[] source = CreateNoisyEdge(
            width,
            height);
        PrismCatalogFilterPlan baselinePlan = CreatePlan();
        ...
        [
            Apply(CreatePlan(strokeLength: 10), source, width, height),
            Apply(CreatePlan(strokeDetail: 12), source, width, height),
        ...
        Assert.True(
            RegionDeviation(coarse, width, 2, 7) <
    ...
    private static PrismCatalogFilterPlan CreatePlan(
        float strokeLength = 6,
        float strokeDetail = 4,
        ...
        PrismCatalogFilterPlanner.Create(
            ...
            [
                ...
                Number(2, relief),
                Number(3, scaling),
                Number(4, strokeDetail),
                Number(5, strokeLength),
                Symbol(6, "Texture", texture)
            ],
