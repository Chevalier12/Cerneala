...
public sealed class RetainedAppAspectContractTests
{
    [Fact]
    public void RetainedAppSampleUsesThemeOrAspectForAtLeastCoreButtonAndTextVisuals()
    {
        ...
        Assert.Equal(UiPropertyValueSource.AspectBase, primaryButton.GetValueSource(Control.BackgroundProperty));
        Assert.Equal(UiPropertyValueSource.AspectBase, primaryButton.GetValueSource(Control.BorderColorProperty));
        Assert.Equal(UiPropertyValueSource.AspectBase, primaryButton.GetValueSource(Control.ForegroundProperty));
        ...
        Assert.True(
            textForegroundSource is UiPropertyValueSource.AspectBase or UiPropertyValueSource.Inherited,
            $"Expected themed text foreground, got {textForegroundSource}.");

        Assert.Contains(Walk(root), element =>
            element is Border border &&
            border.GetValueSource(Control.BackgroundProperty) == UiPropertyValueSource.AspectBase);
    }
