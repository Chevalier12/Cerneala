...
Cerneala SHALL expose metadata-driven invalidation hooks without coupling `UI/Core` to layout, rendering, input, or backend adapters.
...
- **THEN** `UI/Core` does not reference MonoGame, Skia, HarfBuzz, or backend-specific types
